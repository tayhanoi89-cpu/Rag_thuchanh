...
- Ví dụ cấu hình kết nối thường dùng:
  - URI: `neo4j://127.0.0.1:7687`
  - User: `neo4j`
  - Password: `abcd1234`
  - Database: `kb-hops`
- Nếu dùng Neo4j Desktop, cần tạo instance trước, sau đó mở mục `Settings` để kiểm tra port và mật khẩu.
...
- Thực hiện nạp bằng script mẫu có sẵn trong workspace: [neo4j_load_example.py](neo4j_load_example.py).
- Cách chạy:
  1. Đảm bảo Neo4j Desktop đã chạy và database `kb-hops` đã tồn tại.
  2. Chạy lệnh: `python neo4j_load_example.py`.
  3. Kiểm tra kết quả trên Neo4j Browser bằng các câu lệnh Cypher để xem số lượng `Document` và `Chunk` đã được tạo.
...
- Kết nối vào công cụ Neo4j Browser trên máy cục bộ và kiểm tra số lượng các thực thể đã được nạp:
  ...
  - Số lượng quan hệ giữa các tài liệu Document (`RELATIONSHIP`): 8
  - Số lượng nút Chunk: 8848
  - Số lượng quan hệ `PARENT_OF`: 8833
  - Số lượng quan hệ `NEXT`: 7815
  - Số lượng quan hệ `PART_OF`: 8848
